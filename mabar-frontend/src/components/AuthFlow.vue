<template>
  <div class="min-h-screen bg-slate-50 flex items-center justify-center p-4">
    <div class="w-full max-w-md">
      ...
      <div v-else-if="!authStore.user" class="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
        <!-- Header -->
        <div class="bg-emerald-500 px-8 py-12 text-center">
          <div class="w-20 h-20 mx-auto bg-white/20 rounded-full flex items-center justify-center mb-6">
        ...
        <div class="p-8">
          ...
          <div class="flex bg-slate-100 rounded-2xl p-1 mb-8">
            <button
              @click="isLogin = true"
              :class="isLogin ? 'bg-white shadow-sm text-slate-900' : 'text-slate-600'"
      ...
      <div v-else class="bg-white rounded-3xl shadow-xl border border-slate-200 p-8 text-center">
        <div class="w-20 h-20 mx-auto bg-emerald-500 rounded-full flex items-center justify-center mb-6">
        ...
        <p class="text-emerald-600 font-medium mb-8 capitalize">{{ authStore.user.role?.replace('_', ' ') }}</p>
        <button
